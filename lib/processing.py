...
def semantic_chunk_document(
    file_path: pathlib.Path, max_chars: int = 16000
) -> List[str]:
    """
...
async def process_file(repo_url: str, file_path_str: str, status: str) -> None:
    ...
    # Normalize the path to construct clean remote links if desired
    base_link = (
        repo_url.replace(".git", "") if not repo_url.startswith("file://") else repo_url
    )
    # Example: Convert to GitHub/GitLab style blob link
    if "github.com" in base_link:
        remote_file_url = f"{base_link}/blob/main/{file_path_str}"
    else:
        remote_file_url = file_path_str

    # Prepend the structural reference header directly into the text data
    origin_header = f"Source Document Reference: File path `{file_path_str}` in Repository <{remote_file_url}>\n\n"
    compressed_text = origin_header + "\n\n".join([c for c in compressed_chunks if c])
    ...
    if status == "changed":
        await update_document(
            header=file_path_str, content=compressed_text, metadata=metadata
        )
    else:
